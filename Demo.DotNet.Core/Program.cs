...
namespace Demo.DotNet.Core
{
    /// <summary>
    ///     Following program signs the SAML XML and then verifies the signature
    ...
    public static class Program
    {
        public static void Main(string[] args)
        {
            ...
            var signingCertificateSerial = ConfigurationManager.AppSettings["serialNumber"];
            var signingCertificate = GetCertificate(signingCertificateSerial, StoreName.Root);

            // sign the XML with the above certificate
            var signedXml = SignXml(document, signingCertificate);
            ...
            // verify that the XML is correctly signed using the public key of the original certificate
            var validationCertificateSerial = ConfigurationManager.AppSettings["validationCertificateSerial"];
            var validationCertificate = GetCertificate(validationCertificateSerial, StoreName.My);
            var valid = ValidateSignedXml(document, validationCertificate);
        ...
        private static bool ValidateSignedXml(XmlDocument document, X509Certificate2 certificate)
        {
            ...
            // the XML is already signed by the private key of the certificate
            // now check whether the signature is valid using the public key of the same certificate
        ...
        private static X509Certificate2 GetCertificate(string serial, StoreName storeName)
        {
            var store = new X509Store(storeName, StoreLocation.LocalMachine);
            store.Open(OpenFlags.ReadOnly);
            try
            {
                var certificate = store.Certificates.Find(X509FindType.FindBySerialNumber, serial, false)[0];
                return certificate;
            }
            finally
            {
